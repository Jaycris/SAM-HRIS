...
@section('content')
            ...
            <div class="card">
                <div class="card-body">
                        <div class="grid grid-cols-1 gap-4 mb-5 lg:grid-cols-2 xl:grid-cols-12">
                            ...
                            <div id="message" class="text-md text-center xl:col-span-6 ">
                                @if(session('error'))
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full whitespace-nowrap">
                            <thead class="ltr:text-left rtl:text-right bg-slate-100 text-slate-500 dark:text-zinc-200 dark:bg-zinc-600">
                                <tr class="*:px-3.5 *:py-2.5 *:font-semibold *:border-b *:border-slate-200 *:dark:border-zink-500">
                                    <th>ID</th>
                                    <th>Designation Name</th>
                                    <th class="tbldes">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($des as $designation)
                                    <tr class="*:px-3.5 *:py-2.5 *:border-y *:border-slate-200 *:dark:border-zinc-500">
                                        <td class="transition-all duration-200 ease-linear">{{ $designation->id }}</td>
                                        <td hidden class="id">{{ $designation->id }}</td>
                                        <td class="transition-all duration-200 ease-linear">{{ $designation->designation }}</td>
                        ...
                        <div class="flex flex-col items-center gap-4 px-4 mt-4 md:flex-row" id="pagination-element">
                            <div class="grow">
                                <p class="text-slate-500 dark:text-zink-200">Showing <b><b>{{ $des->lastItem() }}</b> of <b class="total-records">{{ $des->total() }}</b> Results
                            </div>

                            <div class="col-sm-auto mt-sm-0">
                                <div class="flex gap-2 pagination-wrap justify-content-center">
                                    {{ $des->links('pagination.custom') }}
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div id="addDesignationModal" modal-center=""
        class="fixed flex flex-col hidden transition-all duration-300 ease-in-out left-2/4 z-drawer -translate-x-2/4 -translate-y-2/4 show ">
        <div class="w-screen md:w-[25rem] bg-white shadow rounded-md dark:bg-zink-600">
            <div class="flex items-center justify-beSTEen p-4 border-b dark:border-zink-500">
                <h5 class="text-16" id="addDesignationLabel">Add Employee</h5>
                <button data-modal-close="addDesignationModal" id="addDesignation"
                    class="transition-all duration-200 ease-linear text-slate-400 hover:text-red-500"><i data-lucide="x"
                        class="size-5"></i>
                </button>
            </div>
            <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
                <form action="{{ route('designationSave') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div id="alert-error-msg"
                        class="hidden px-4 py-3 text-sm text-red-500 border border-transparent rounded-md bg-red-50 dark:bg-red-500/20">
                    </div>
                    <div class="grid grid-cols-1 gap-4 xl:grid-cols-13">
                        <div class="xl:col-span-6">
                            <label for="lblDesignation" class="inline-block mb-2 text-base font-medium">Designation</label>
                    ...
                    <div class="flex justify-end gap-2 mt-4">
                        <button type="reset" id="close-modal" data-modal-close="addDesignationModal"
    ...
    <div id="updateModal" modal-center=""
        ...
        <div class="w-screen md:w-[25rem] bg-white shadow rounded-md dark:bg-zink-600">
            <div class="flex items-center justify-beSTEen p-4 border-b dark:border-zink-500">
                <h5 class="text-16" id="updateModalLabel">Edit Designation</h5>
            ...
            <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
                <form action="{{ route('designationUpdate') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div id="alert-error-msg"
                        class="hidden px-4 py-3 text-sm text-red-500 border border-transparent rounded-md bg-red-50 dark:bg-red-500/20">
                    </div>
                    <div class="grid grid-cols-1 gap-4 xl:grid-cols-13">
                        ...
                        <div class="xl:col-span-6">
                            <label for="lblDesignation" class="inline-block mb-2 text-base font-medium">Designation</label>
                            <label for="lblrequired" class="inline-block mb-2 text-base font-medium required-asterisk">*</label>
                            <input type="text" id="e_designation" name="designation" value=""
                                class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                placeholder="Designation Name">
                        </div>
                    </div>
            ...
            <div class="max-h-[calc(theme('height.screen')_-_180px)] overflow-y-auto px-6 py-8">
                ...
                <div class="mt-5 text-center">
                    ...
                    <div class="flex justify-center gap-2 mt-6">
                        ...
                        <form action="{{ route('designationDelete') }}" method="POST">
                            @csrf
